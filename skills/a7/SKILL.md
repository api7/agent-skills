...
name: a7
description: >-
  Configure and operate API7 Enterprise Edition (API7 Gateway) through the a7 CLI. Use whenever
  the user wants to create, inspect, change, or delete API7 EE routes, services, upstreams,
  consumers, credentials, SSL certificates, global rules, or plugins (ai-content-moderation,
  ...
  personas and the a7 command conventions.
version: "2.0.0"
license: Apache-2.0
metadata:
  author: API7.ai Contributors
  cli: a7
  cli_repo: https://github.com/api7/a7
  product: API7 Enterprise Edition
  apisix_version: ">=3.0.0"
  references: 40
---

# a7 — API7 Enterprise Edition agent skill

`a7` is the command-line tool for API7 Enterprise Edition. It wraps the API7 EE control-plane API and the Admin API of a gateway group. Every runtime resource is scoped to a gateway group (`--gateway-group` / `-g`).
This skill is a router: it tells you which reference file to read for the task at hand. Read only what the task needs.

## 1. Always start here

Read [references/shared.md](references/shared.md) once per session before running any `a7` command. It covers the noun-verb command pattern, output formats, context/authentication handling, resource types, and the declarative `a7 config sync` workflow. Every other reference assumes it.
...
|---|---|---|
| `ai-aws-content-moderation` | [plugins/ai-content-moderation.md](references/plugins/ai-content-moderation.md) | AI content moderation plugins. Covers both ai-aws-content-moderation (AWS Comprehend, request-only) and… |
...
| `ai-prompt-template` | [plugins/ai-prompt-template.md](references/plugins/ai-prompt-template.md) | Covers defining reusable prompt templates with variable placeholders, enforcing prompt structure, and… |
...
| `consumer-restriction` | [plugins/consumer-restriction.md](references/plugins/consumer-restriction.md) | Covers restricting access by consumer name, service ID, or route ID using whitelist/blacklist modes and… |
...
| `http-logger` | [plugins/http-logger.md](references/plugins/http-logger.md) | Covers pushing access logs to HTTP/HTTPS endpoints in batches, custom log formats, and gateway group scoping. |
...
| `kafka-logger` | [plugins/kafka-logger.md](references/plugins/kafka-logger.md) | Covers pushing access logs to Apache Kafka topics, broker configuration, SASL authentication, and gateway… |
...
| `limit-count` | [plugins/limit-count.md](references/plugins/limit-count.md) | Covers fixed-window rate limiting, count/time_window configuration, key types, Redis and Redis-cluster… |
...
| `openid-connect` | [plugins/openid-connect.md](references/plugins/openid-connect.md) | Covers OIDC authorization code flow, bearer token validation, token introspection vs JWKS verification,… |
| `prometheus` | [plugins/prometheus.md](references/plugins/prometheus.md) | Covers enabling Prometheus metrics export on routes and globally, exposed metrics (HTTP status, latency,… |
...
| `skywalking` | [plugins/skywalking.md](references/plugins/skywalking.md) | Covers distributed tracing with Apache SkyWalking OAP, sampling configuration, service topology, and gateway… |
...
|---|---|---|
| `api-versioning` | [recipes/api-versioning.md](references/recipes/api-versioning.md) | Covers URI path versioning, header-based versioning, traffic splitting for gradual migration, and version… |
| `blue-green` | [recipes/blue-green.md](references/recipes/blue-green.md) | Blue-green deployments. Covers creating two service-backed environments, switching traffic via route… |
| `canary` | [recipes/canary.md](references/recipes/canary.md) | Covers gradual traffic shifting with the traffic-split plugin, header-based canary routing, monitoring… |
| `circuit-breaker` | [recipes/circuit-breaker.md](references/recipes/circuit-breaker.md) | Covers the api-breaker plugin, unhealthy thresholds, healthy recovery, response code classification, and… |
| `graphql-proxy` | [recipes/graphql-proxy.md](references/recipes/graphql-proxy.md) | Covers operation-based routing, per-operation rate limiting, REST-to-GraphQL conversion, and enterprise… |
| `health-check` | [recipes/health-check.md](references/recipes/health-check.md) | Backend health checks. Covers active health checks, passive health checks, combining both, healthy/unhealthy… |
...
| `multi-tenant` | [recipes/multi-tenant.md](references/recipes/multi-tenant.md) | Covers gateway-group isolation, consumer policies, service-backed tenant routes, and credential-based tenant… |
...
1. **Inspect before you change.** List or get the current resource (`a7 route get <id>`, `a7 route list`) and show the user what exists.
...
3. **Apply only the approved change**, scoped as narrowly as possible (always pass `--gateway-group`).
4. **Verify** with a follow-up `get` and, where possible, a test request through the gateway.
5. **Keep a rollback path**: note the previous configuration before updating, and use `a7 config dump` / `a7 config diff` for larger changes.
6. **Never put an access token or Admin API key in a prompt, file, or commit.** Use `a7 context` for credentials.
7. **Use a non-production gateway group for a first run** of any new workflow.
...
2. Inspect: `a7 route get orders -g default`.
3. Propose the merged plugin block, get approval, apply with `a7 route update orders -g default -f route.yaml`, then verify with `a7 route get orders -g default`.
